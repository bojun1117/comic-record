...
REST API 規格,前後端共同 source of truth。
...
| 環境 | URL |
|---|---|
| 本機 dev | `http://comic-vibe.localtest.me`(K8s ingress) |
| prod | (尚無正式部署) |
...
| 413 | `PAYLOAD_TOO_LARGE` | `request body exceeds 64kb` | ingress 預設限制以內就好 |
| 429 | `RATE_LIMITED` | `too many requests` | 加 rate limit 後才會出現 |
| 500 | `INTERNAL_ERROR` | `internal server error` | DynamoDB / SSM / 程式 panic |
| 503 | `SERVICE_UNAVAILABLE` | `service temporarily unavailable` | readiness probe 偵測 DB 連不到時回 |
...
`web/.env.development` 的 `VITE_API_BASE_URL` 指向 K8s ingress(`http://comic-vibe.localtest.me`)。
